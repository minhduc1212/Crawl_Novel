...
def change_tab(window_handles, current_window_id):
...
for url in urls:
    try:
        ...
        change_tab(window_handles, current_window_id)
        ...
        with open(f"E:/LT/Crawl Truyện Chữ(Python)/Các Nàng Đều Muốn Làm Nữ Chính Của Ta/{chap_name_area_text}.txt", "w",
                  encoding="utf-8") as f:
            f.write(content.text)
            sleep(1)

    except Exception as e:
        print('Lỗi khi xử lý URL:', e)
        errors.append(url)
